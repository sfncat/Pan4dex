...
> 背景：本仓最初就是 **Linux 版**（早期设计稿的标题即「Pan4dex 万格 Linux 版」，为
> Ubuntu/Kali 写，后来才对齐 Windows 的 Q-Dir；那份稿子已因大面积失真删除，见
> `git log --diff-filter=D -- docs/design.md`）。本文回答一个问题：**现在这些能力在
...
10. **v1.9.019：L5 与 L6 闭环，代价是撞出「只有 vim 不行」这种最难查的一类缺陷**。同一个内嵌
   终端 dock 里 htop、less 都正常，只有 vim 完全不显示（进程起得来、`:q!` 退得掉，输入链全好）；
   定案靠的是产物自己的 stderr 里那**唯一一条** traceback —— pyte 处理 vim 发的 `CSI ? … m` 时抛
   `TypeError`，`Stream.feed` 顺手丢掉整段输出，vim 的首屏就此没了。两层容错 + 37 项用例，
   真机复验 vim 首屏与状态行已画出来。见 gotchas 第 52 条。
11. **v1.9.020：L2（万条目真共享）在用户真 NAS 上逐条量完了，三条判据两条不成立**。
    先说成立的：慢位置判据在真 CIFS 上 26/26（含不挂 watcher、永久删除文案、跨服务器判跨卷）；
    而「不闪」要靠一个 **A/B 才说得清**：SMB 1 万条目端到端 13.6s（纯扫描 10.08s 全在 worker
    线程），主线程最大停摆 1256ms；本地 ext4 **同条数**对照扫描只 0.23s、可停摆仍是 1256ms ——
    那一下是「1 万行落地」的通用代价，不是网络的锅（产物版在 `:10` 真 X 上同样能开能排能滚，
    状态栏「200 个目录, 9800 个文件」）。不成立的两条：**没有枚举取消入口**
    （`_LoadTask.run()` 里 `enumerate_dir()` 无取消检查），以及**切走窗格后仍把盘扫完** ——
    被放弃的扫描在切走后 23.2s 返回 10,000 条并被采纳，在主线程阻塞 1388ms（用户此时已经在看
    另一个目录），`gen` 只挡污染、不挡耗盘。
    本轮最大的成本花在探针自己身上：同一个「排序没生效」的现场，先后被误判为产品缺陷两次，
    而两次都是取样取错了行（前 200 行全是目录，而目录在「大小」列不比 size；降序时同类内部
    还会反转）—— 见 gotchas 第 53 条。
12. **v1.9.021（修复）：上条那两条不成立的缺口已闭合，L2 升 ✅**。产品侧给枚举链
    接上了取消：`enumerate_dir(cancel=...)` 经 `_Enumerator` 每 256 项探一次令牌，令牌就是
    `_LoadTask` 自己（`__call__`=「该停了吗」，一并校 gen 与 `abandoned`）；命中抛
    `_EnumerationAborted`，`run()` **不回投条目、只投 `cancelled`**。导航离开慢位置时
    `set_directory` 后调 `_drop_stale_scans`，**复用 `mounts.is_remote_location` 慢位置判据**
    （经 `_is_network`）只放弃离开那个目录的在飞扫描，本地目录不中断；另有显式入口
    `DirStoreModel.cancel_load(path)`。趁 NAS 上 big10k 靶子还在，用 SMB big10k 与本地 ext4
    `/home/kali/big10k` 同条数 A/B 复跑 p59e/p59f：[C] 被放弃的枚举在切走后 **0.24s 停扫**、
    经 `cancelled` 回报未采纳，心跳最大 10.3ms、视图仍是小目录 1 行（旧基线：切走后 23.2s
    返回并被采纳、主线程阻塞 1388ms）。**但有一条未动**：p59f 前景「盯着目录」时 1 万条目
    采纳仍停摆 ~1.2s（SMB 1220ms vs 本地 ext4 1209ms，扫描差 47 倍而停摆同量级）——那是
    「1 万行落地」的通用代价，只能靠分批插入压下，与取消链路无关，另案。真 NAS/真共享上
    的验证仍需 230（`:10` 真 X），Windows 主机只能跑 offscreen 与用例。
...
| HEIC/HEIF 预览 | 打包带 `_pillow_heif` | 同一套：`packaging/Dockerfile-linux:61` 的 pip 列表里已有 `pillow-heif`，v1.9.015 起的产物实测带 `_pillow_heif*.so`（见 §3.2）；真机渲染见 §5.2 L7 | 🟢 | `widgets/thumbnail_view.py:24-37`、`packaging/Dockerfile-linux` |
...
| 右键「打开方式」列表 | 注册表 ProgID / OpenWithList MRU | `freedesktop .desktop` 解析 + mime 匹配 + Exec 字段码剥离；内置兜底改为**与 Windows 共用一道门**（v1.9.014） | ✅ 枚举与**真把文件交给外部程序**都已真机闭环（L5，p50） | `core/open_with.py`（`needs_builtin_topup()` / `_list_linux`）|
...
> L6 一次过五项：F4 开 dock、`$SHELL`=zsh、`$TERM`=xterm-256color、`stty size` 与 dock 尺寸一致、
> **缩放主窗口后 PTY winsize 跟着改**（11 316 → 10 171）、关 dock 会话保活（设计如此），
> 而且在真 vim 里按 F7 **不弹「新建文件夹」框**（v1.9.017 那条守卫的作用面自证）。
> 唯一红项是 **vim 完全不显示**：`pgrep -x vim`=1、`:q!` 退得掉（输入链全好），但 dock 里一直
...
| L2 | 挂 SMB（gvfs 或 CIFS）后浏览 | 打开 1 万个条目的共享目录：不闪、能取消、切走窗格不继续扫；慢位置判据在 POSIX 上是否真生效 | ✅ **v1.9.020 在用户真 NAS 上逐条实测（p57/p58/p59 系列/p60 系列）+ v1.9.021（7c4789b）补齐取消能力**。靶子：`backupSpaceInJKJ/pan4dex/big10k`（CIFS/SMB3.1.1，10,000 条目 = 9,800 文件 + 200 目录，100M，p58b 造）。原判据里「不在用户 NAS 上造 1 万个文件」是**用户本轮明确授权**后才造的，验完删（清理脚本 `tmp/p58z_cleanup_nas_target.py`，四道闸：挂载点仍在 + 产品判据判为慢位置 + 目录名匹配 + 条目数 ≥ 9000，任一不过就 exit 2 不动一个文件）<br>**「按本地目录处理」已不成立**：p57 在真 CIFS 上 26/26 —— 挂载根/带尾斜杠/挂载内空子目录/第二台服务器全判为慢位置，本地 ext4、家目录、`/proc`、`/sys`、`/`、空串、相对路径全判本地；网络目录不挂 watcher；删除文案是「永久删除…网络位置没有回收站」；两台服务器 `st_dev` 63/74/2051 正确判跨卷 → copy/move 决策对<br>**不闪（源码版）**：p59f 全栈前景心跳 —— SMB 1 万条目端到端 13.6s，其中**纯扫描 10.08s 全在 worker 线程**，主线程最大停摆 **1256ms**；**本地 ext4 同条数对照（`/home/kali/big10k`，扫描只 0.23s）最大停摆也是 1256ms** → 这一下是「1 万行落地」的通用代价，与 SMB 无关（裸模型口径：p59 [A] 心跳最大间隔 21.2ms、RSS 42.6MB、一次导航只枚举 1 次）<br>**不闪（产物版）**：p60c/p60d/p60e 在 `:10` 真 X 上跑 `pan4dex-1.9.020-linux`，真键盘 `Ctrl+L` 打路径 → 1 万行画出、状态栏「**200 个目录, 9800 个文件**」、`Ctrl+End` 跳到最后一行 `f00000.txt`；重扫期间在另一窗格按方向键，三张连拍各不相同 = 主线程照常接输入<br>**列头排序在 1 万条目 SMB 上确实作用到条目**（1.8 那条 🟢 至此才算真验过）：p59e 读**全部** 1 万可见行逐条断言单调 —— 大小降序（目录块按名称、文件按字节不增）/ 修改日期升序 / 名称升序 三条全绿，排序期 **0 次文件系统查询**（gotchas 29 条守住了），lessThan 各 8.6–11.5 万次；产物版真鼠标点「名称」表头两次 → 画面从 `d00000` 起翻成 `d00199` 起（裁图核对）<br>✅ **「能取消」**（v1.9.021 修，7c4789b）：`enumerate_dir()` 加取消令牌，`_LoadTask` 自身即令牌（`abandoned` + gen 校验），扫描每 256 项探一次、命中抛 `_EnumerationAborted`，不回投半截列表 —— 见 gotchas 第 54 条<br>✅ **「切走窗格不继续扫」**（v1.9.021 修）：`set_directory` 切换后 `_drop_stale_scans` 复用 `mounts.is_remote_location` 放弃在飞扫描。真 NAS A/B（p59e/p59f）：被放弃的枚举切走后 **0.24s** 停扫、未采纳、心跳最大 10.3ms；修前是切走后 **23.2s** 返回 10,000 条且**被采纳**（`adopted=True`），采纳在主线程阻塞 **1388ms**、同批处理造成一次 **3163ms** UI 停摆<br>⚠️ 未深查的附带观察：产物里单按 `End` 不跳列表末尾（`Ctrl+End` 会），源码无 `Key_End` 处理，不在 L2 判据内，也还没进 unsolved-issues |
...
| L7 | 图标与缩略图 | 确认「所有文件同图标」的实际观感；SVG/PNG/HEIC 缩略图是否出得来（qsvg 插件、pillow-heif） | 🟡 **图标那半仍未修**：非目录文件确实共用同一个通用图标（观感与 Windows 一致地差，属第三批的 `QFileIconProvider` 范围，见 §7）。✅ **HEIC/PNG 缩略图那半已闭环**（11a0eec / p56）：Qt 原生不认 `.heic`（`QImageReader.canRead=False`），走 Pillow + `pillow-heif` 回退成功，v1.9.019 产物在真机上截图亲眼看到图像渲染，并有用例 `test_heic_thumbnail_in_xlarge_mode` 断言「不是白纸文件图标 + 采样 ≥3 种颜色」 |
...
- ✅（已闭环）`docs/feature-checklist.md` 18.4「支持 7z/rar」原标 🔴，实际 `core/archive_ops.py`
  早已通过 7z 命令行支持 —— 0f9e6e9 已把该行改为 🟢，本条记账结清。
- ✅（已重写）`AGENT.md` 曾整体落后（技术栈写 Python 3.10+、目录结构缺 `dir_model.py` 等十余个
  模块、版本号规则仍写 `0.9.5XX`、构建入口指向已废弃的 `build.sh`、机器表停在 win54/55）。
  2026-09-23 起 `AGENT.md` 改为**纯索引 + 铁律**，正文一律指向本文档与 `architecture.md`，
  不再复制细节 —— 复制一份就会过期一份。
