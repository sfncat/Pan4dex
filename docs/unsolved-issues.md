...
>
> 2026-09-23 追更：① 问题 13 的定性补进 v1.9.014 的随机段错误排查，结论仍是「已收敛、
> 未证根除」，所以本节标题不再写「已解决」；② **问题 14（pyte 不实现备用屏，vim 退出后
> 残留 `~` 与状态行）自 v1.9.019 记录至今没有修复提交**，仍挂在「⏳ 未解决」；③ v1.9.015
> 之后的 Linux 真机验收结论请去看 `docs/linux-gap.md` §5.2，不要以本文件的日期为准；
...
### 问题 14：内嵌终端没有备用屏，vim 退出后满屏 `~` 残留（Linux）

**现场**（linux230 真机，p53 截图 `q3_after_vim`）：在终端 dock 里跑 vim，`:wq` 退出后，
dock 上留下的还是 vim 的 `~` 行与状态行，而不是退出前的 shell 屏幕；真实终端（xterm/VTE）
在这里会恢复主屏内容。

**根因**：pyte 压根没有备用屏概念。`\x1b[?1049h` / `\x1b[?1049l` 会被 `set_mode`/`reset_mode`
收下，但只是把 `1049 >> 5 == 32` 这个位记到 `screen.mode` 里（p52b 实测私有模式集合
`[32, 224, 800, 64128]`），**没有任何地方消费它**；`pyte/modes.py` 里也找不到 ALT/1049。

**为什么先不做**：v1.9.019 已把「vim 完全不显示」这条主坑填了（gotchas 第 52 条），剩下的
只是退出后的画面整洁度：输入链、存盘、退出全部正常，程序自己也不会因它误判终端能力。
要做就是在 `TerminalScreen` 里补一套屏幕缓冲（进备用屏时快照、退出时还原），与
`HistoryScreen` 的历史/分页逻辑纠缠，风险远大于收益。留待真的有人被它碍到再做。
（htop 也走备用屏，但本轮没专门观察它退出后的残留 —— 只能断言 vim 那一条。）

---

## ✅ 已收敛（v1.9.004 —— 未证根除，仍在观察，见本节末尾的量化口径）
...
**相关文件**：
- `main.py` — `_cli_output()` 简化为 print；当年新增的 `hide_console_if_standalone()` **现已改名
  换实现**为 `free_console_in_gui_mode()`（`main.py:473`，思路从「隐藏窗口」变成「GUI 模式下
  释放控制台」）；`main()` 中 GUI 初始化前调用它
