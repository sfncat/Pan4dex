...
#### 共享侧边栏（默认）
- 一个全局目录树侧边栏（`TreeSidebar`），4 个窗格共用
- 点击目录树节点 → 导航到当前活动窗格
...
#### 每窗格独立目录树
- 每个窗格左侧各挂一个独立目录树（`PaneTreeView`）
- 路径栏中有一个目录树按钮（🌲），点击只控制本窗格的目录树
- 每个树独立维护展开状态和选中状态
- 点击某个窗格的目录树 → 仅影响该窗格
- 默认隐藏，需要时按按钮打开，不需要时再按关闭
...
#### 使用方式
- 共享侧边栏：`Ctrl+Shift+T` 切换
- 每窗格独立：点击路径栏的 🌲 按钮，独立控制每个窗格
- 两种模式可以同时使用（侧边栏打开时，窗格内的树也可以打开）
...
│   ├── tree_sidebar.py      # 目录树侧边栏（全局共享）
│   ├── pane_tree_view.py    # 窗格内嵌目录树（每窗格独立）
│   └── filter_bar.py        # 筛选栏
├── config/
│   ├── __init__.py
