...
| 13.10 | 添加到收藏夹 | P2 | 🟢 | 选中目录后右键添加到收藏夹 | BookmarkSidebar.add_bookmark_with_path() | pane.py |
...
| # | 功能 | 优先级 | 状态 | 验证方式 | 说明 | 实现情况 |
|---|---|---|---|---|---|---|
| 16.1 | 文本比较 | P1 | 🔴 | 对比两个文本文件，高亮差异 | difflib + QTextEdit | **坏的**：`compare_text()` 调 `self.highlight_diffs()`（`file_compare.py:316`），全仓没有这个定义 → 文本比较必抛 `AttributeError`，被外层 `except` 吞成一句 `QMessageBox.warning("比较失败：...")`（模态框；2026-09-23 探针实测）。而且 `FileCompareDialog(file1, file2)` 构造末尾就直接 `compare()`，所以「带两个文件构造」这个用法一打开就弹错误框，offscreen 下表现为**永久挂住**（没人点 OK）。登记在第 23 节 **T4** |
| 16.2 | 二进制比较 | P2 | 🟢 | 逐字节对比两个文件 | 分块读取、十六进制转储、差异分组 | `file_compare.py compare_binary()` + `show_binary_results()` —— 2026-09-23 探针实跑通：绕开文本模式、直接 `compare_mode=BINARY` 调 `compare()`，`binary_result` 正常出报告。**前提是别走 16.1 那条路**，否则先撞上 T4 |
| 16.3 | 比较结果导出 | P2 | 🔴 | 导出比较结果为 HTML/文本 | 报告生成，带元数据 | **HTML 那半是坏的**：`_export_html()` 调 `self.escape_html()`，同样是未定义方法 → 导出 HTML 直接 `AttributeError`；`_export_text()` 未发现同类问题但也没跑过。与 16.1 一起登记在 **T4** |
...
| 18.4 | 支持 7z/rar | P3 | 🟢 | 通过 7z 命令行支持更多格式 | 自动检测 7-Zip/WinRAR | archive_tool.py _find_7z() + _find_rar()（解压侧另见 `core/archive_ops.py`） |
...
| # | 功能 | 优先级 | 状态 | 验证方式 | 说明 | 实现情况 |
|---|---|---|---|---|---|---|
| 21.1 | 自定义操作 | P3 | 🔴 | 用户定义快捷操作（如打开编辑器、转换格式） | JSON 配置 + 对话框 | `config/user_operations.py`（配置层）可用且有测试；`widgets/user_operations_dialog.py` **连 import 都失败** —— 模块第 4 行从 `PyQt6.QtWidgets` 导入 `QKeySequenceValidator`，这个名字在 QtWidgets 和 QtGui 里都不存在，`ImportError` 是硬错（2026-09-23 实跑）。所以它不是「建好没接线」，是**没建好**：全仓除测试外零引用，那 5 条测试全红。登记在第 23 节 **T2** |
| 21.2 | 操作快捷键 | P3 | 🔴 | 为自定义操作绑定快捷键 | 快捷键配置框架 | 未实现：`user_operations.py` 与对话框里没有任何 shortcut/hotkey 代码，21.1 的入口也没接上。与入口一起登记在 **T2** |

## 22. 目录树与窗格内标签页

> 本节自初版（e109d9b）起就缺表标题、行首还多了一截 `||`，2026-09-23 补上标题并把行规范成普通表格行；编号 22.x 未动，changelog 里的历史引用仍然对得上。

| # | 功能 | 优先级 | 状态 | 验证方式 | 说明 | 实现情况 |
|---|---|---|---|---|---|---|
| 22.1 | 目录树侧边栏 | P1 | 🟢 | 左侧显示目录树，双击导航到当前活动窗格 | QTreeView + QFileSystemModel | tree_sidebar.py TreeSidebar |
| 22.2 | 活动窗格跟踪 | P1 | 🟡 | 焦点在哪个窗格，目录树导航就作用于哪个窗格 | 点击导航走 `MainWindow.on_tree_folder_clicked`→`target_pane()`（正确）；但右键菜单与「跟随当前目录」仍直接判 `_active_pane` | tree_sidebar.py:121 `_show_context_menu`、:180 `on_follow_clicked`（待办 T3） |
...
| 2026-09-23 | **全仓文档审计（AGENT.md 索引化那一轮）+ 跑代码复核，回写本表**：①**16.1 文本比较 🟢→🔴、16.3 HTML 导出 🟢→🔴**（`file_compare.py:316` 调不存在的 `highlight_diffs()`、`:578` 等调不存在的 `escape_html()`，`0f9e6e9` 起就没跑过；16.2 二进制实测可用，留 🟢）；②**21.1 🟢→🔴**、21.2 🟢→🔴 —— `widgets/user_operations_dialog.py:4` 从 `PyQt6.QtWidgets` 导入不存在的 `QKeySequenceValidator`，模块 import 即 `ImportError`，配套 5 条用例全红，且全仓除测试外零引用；③新增待办 **T2**（先修 import 再谈接入入口与快捷键）、**T4**（file_compare 两个幽灵方法）、**T5**（全量测试自 v1.9.020 起跑不完，最后一条全量绿记录停在 v1.9.019）；④审计中另查到 **T3**：`widgets/tree_sidebar.py:121`、`:180` 仍直接判 `_active_pane`，是 gotchas 47/49 那一类在 Linux/X11 上会静默失灵的落点，22.2 因此从 🟢 改 🟡；⑤第 22 节补齐自初版就缺的表标题、把行首多出来的那截竖线规范成普通表格行，13.11 改号 13.10（13.10 从未存在过，纯编号断档）。**产品代码与测试本轮一行未动**，全部是登记与回写 | - |
