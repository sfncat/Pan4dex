...
### v1.9.018 — 2026-09-17（开发分支 dev/shell-behavior-smb-perf）

> 真机 GUI 验收第三轮（p41）+ 一次顺着「删除为什么没落地」摸下去的代码路径探针
> （p42）。第三轮的收获一半是好消息（回焦与守卫都在真机自证生效、L10 首次命中），
> 一半是：`delete()` 里还藏着最后一处平台假设泄漏 —— 上一轮把删除**文案**的网络
> 分支去掉了 `nt` 门控，却没动**行为**那一半。
...
- Windows 全量：定序 + 随机各一轮 **624 passed / 5 skipped**
