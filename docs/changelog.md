...
### v1.9.023 — 2026-09-23（开发分支 dev/shell-behavior-smb-perf）

> 修一个用户现场报的粘贴缺陷：**在系统文件管理器里复制了照片，回 pan4dex 粘贴，粘出来的却是早先在 pan4dex 里复制的文件**。

#### 🐛 缺陷修复：外部程序后复制时，粘贴改用系统剪贴板的新内容
- 现象：pan4dex 内复制文件 → 去资源管理器复制一张照片 → 回 pan4dex Ctrl+V →
  粘出的是 pan4dex 里那个老文件，不是照片（本机 v1.9.022 产物上报告）
- 根因：`_paste_to` 无条件「应用内剪贴板优先，为空才读系统剪贴板」；而应用内复制/剪切
  会同步写回系统剪贴板（供外部粘贴用），外部程序后复制时只改得动系统条、清不掉应用内
  SHARED_CLIPBOARD 残留 —— 粘贴永远命中过期内部条
- 修法：粘贴一刻做裁决（新抽 `Pane.choose_paste_source`）：应用内条非空时先比对系统条的
  文件列表 —— 一致 = 内部条仍是最后一次复制，沿用内部动作语义（内部剪切的 move 不能靠
  回读 Preferred DropEffect 判，会误判成 copy）；不一致 = 只能是外部后复制（应用内必然同步
  两边），改粘系统内容并就地作废内部条；系统剪贴板无文件条（纯文本/图片）时维持旧优先级
- 测试：新增 3 条裁决用例（现场复现路径 / 一致时保留 cut 语义 / 无文件条回退），
  `test_phase1_shell_behavior.py` 16 passed；`regression + pane_dir_store + pane_target +
  drop_action` 53 passed。跨进程剪贴板时序需装包后手工验证一次
...
#### ✅ 真机自证（p53 源码版 + p54 产物版）
- **p53（改完的源码，不等出包）**：运行日志里 Traceback **0 次**（改前恒为 1）
- **p54（v1.9.019 Linux 产物，`releases/pan4dex-1.9.019-linux`）**：产物 stderr 里
  Traceback **0 次**；`TERM=xterm-256color`、`stty size` 11 318
- **vim 真画出来了**（两轮都是亲眼核对裁图，不是拿脚本判定）：`aaa/bbb/ccc` + `~` 列 +
  状态行 `"~/p5x/vimfile.txt" 3L, 12B  1,1 All`；在 vim 里插入一行 `UNIQUE-TEXT-9137`
  （p54 为 `UNIQUE-TEXT-9154`）后 `:wq`，磁盘上的文件内容真的是它；退出后回到 zsh
  提示符，并打出 `3L, 28B written`；htop 首屏照常画出（它退出后的残留本轮
  未专门观察，见问题 14）
- ⚠️ **本轮推自己的判据**：验收脚本里那条「三张连拍截图不再完全相同」靠不住 ——
  p54 得到 272714/272714/272714（脚本报 🟡）而 vim 明明画出来了，因为静止的 vim
  屏幕本来就不变；改前可疑是因为那张定格画面**内容是 shell 历史行**。已记 gotchas 第 52 条
- **Windows 产物冒烟**：`pan4dex.exe --version` → `Pan4dex v1.9.019 (build 2026-09-18 08:32:56)`，
  连起两轮均存活，崩溃日志里只有 2 条「启动于」标记、无异常
...
> 自证生效、L10 首次命中），一半是：`delete()` 里还藏着最后一处平台假设泄漏 —— 上一轮
> 把删除**文案**的网络分支去掉了 `nt` 门控，却没动**行为**那一半。本版修完它，并在
> 真机上把「本地 / 网络 / Shift+Del」三条删除链全部跑绿（p46：网络位置那一条不再在
> 共享根长 `.Trash-1000/`，就是本版的自证）。

#### 🐛 缺陷修复：网络位置的删除只说不做 —— 文案承诺永久删除，代码却丢进了回收站
- **真机现场**（p42，230 上的 CIFS 共享）：删一个文件，确认框写「网络位置的 1 个项目将
  被永久删除、无法恢复（网络位置没有回收站）」，而实际发生的是 `send2trash` 在**共享根
  目录**凭空建了一个 `.Trash-1000/`（`files/` + `info/`），把文件藏进用户的 NAS：既不是
  文案承诺的「已删除」，也不是任何桌面能看到的回收站
- **根因**：v1.9.013 把 `describe_removal()` 的网络分支从 `os.name == 'nt'` 里放出来时，
  `FileOperations.delete()` 里「走不走回收站」那扇门还锁在 nt 上（POSIX 分支就一句
  `send2trash(path)`）。「措辞与行为共用一个判据」这件事只改了对称的一半
- **修法**：`delete()` 的第一档改成不分平台的 `_is_network_path(path)` → 直接
  `os.remove` / `shutil.rmtree` 并计入 `permanent_fallbacks`（结果里那句「N 个项目位于
  网络位置，已永久删除」现在两端都会说）。Windows 原有的 `\\?\UNC\` 前缀还原与
  `normpath` 一字不动地保留，并专门留一条用例钉住它
- **顺手两处**：`import send2trash` 挪到真要进回收站时才做（网络位置那档不该依赖它）；
  打掉一段死代码 —— 旧的 `except OSError: if _is_network_path(...)` 兜底，判据命中时
  已经走第一档，永远落不到那里

#### 📝 记录（未改）：删除确认框的默认按钮是 No
- 资源管理器里回车 = Yes；这里是回车 = 取消。**p41 段 6 报的「Linux 回收站没落地」就是
  被这个默认值坑出来的假故障**（p42 用产品同一条代码路径证明本地 `send2trash` 完全正常：
  `~/.local/share/Trash/{files,info}` 该建的建、该搬的搬）
- 要不要为了对齐资源管理器把它翻成 Yes，是一个待决问题（永久删除的确认框默认「删」
  风险不小），记在 `docs/linux-gap.md` §5.2 待拍

#### ✅ 真机自证（p43 第四轮 / p46 第五轮）：三条删除链全部跑绿
- **本地 `Del`** → 确认框「移到回收站」+ 文件真出现在 `~/.local/share/Trash/{files,info}`
- **CIFS `Del`** → 文件消失、共享根**不再**长 `.Trash-1000/`（v1.9.018 的那条修复就此自证），
  并弹「删除完成 / 1 个项目位于网络位置，已永久删除（无法恢复）」—— 就是 `permanent_fallbacks`
  那句新文案（旧行为在 Linux 上从不计它）
- **`Shift+Del`** → 弹「确认永久删除」+ 文件消失且回收站里没有它
- 断言方式换了：拿**对话框标题当 oracle**（`xdotool search --onlyvisible --name '^确认删除$'`）
  而不是比对截图字节数 —— p43 就是被后者的盲区骗了一整轮（模态告知框会让截图变、
  却让按键全部失效），见 gotchas 第 51 条

#### 🧪 测试与两端发布
- Linux 全量（已提交状态，定序 + 随机各一次）**623 passed / 6 skipped**，与 Windows
  624/5 总数 629 吻合；docker（`py311` 镜像）出包成功，产物 `--version` 正常、
  offscreen 下 10s 仍存活
- Windows 出包 `releases/pan4dex-1.9.018`（148.1 MB / zip 57.8 MB）+ 冒烟两次 12s 存活

#### 📐 真机数据（ln5 L9 全盘搜索闭环：p46 §4 + p48 A/B）
- 搜索对话框在真机上跑起来：`/home/kali/pics` + `*` → 「搜索完成，共找到 3 个文件」，
  结果列表正常；空目录与不存在的目录都有明确中文校验（「请输入搜索目录」/「目录不存在」）
- **中途停止靠 A/B 坐实**：同一查询（`/home/kali` + `*.txt`，全盘 130 万文件、纯遍历 57.8s）
  3s 就点停止 → **503** 项，不点 → **40,944** 项（`find` 独立数得 40,933，吻合）
- ⚠️ 停止与跑完的状态栏文案**一模一样**（都是「搜索完成，共找到 N 个文件」）——
  用户看不出屏幕上这是一份部分结果（未改，待拍）
- ⚠️ 搜 `/` 的后果（p49 危害探针）：确实会进 `/proc`、`/sys`、`/dev`。只按名字筛时无害
  （/dev 623 项 0.01s、/proc 212,416 项 5.25s、/sys 49,011 项 1.15s 均自然结束），但**一勾选
  内容搜索，worker 就卡在伪文件的 `read` 里且 `stop()` 叫不回来**（实测分别卡在
  `/dev/ptmx`、`/proc/<pid>/task/<pid>/fd/9`、`/sys/kernel/security/apparmor/revision`，
  `stop()` 后 3s 仍 `isRunning()`）—— 因为 `stop()` 只在遍历循环里查 `_stop`，
  卡在系统调用里时它根本没机会查。这是 v1.9.019 的头号候选缺陷，本版只记录
- ⚠️ 网络位置删完后的模态「删除完成」告知框：资源管理器没有这一步，而它会把后续
  所有键盘操作吞掉（自动化与真用户都会碰到）；高级搜索对话框是模态的 → 开着时
  `Ctrl+Q` 退不掉（挂账项 20.5「搜索窗口改非模态」的又一条理由）

#### 🧪 用例（本版主交付）
- `tests/test_m2_file_operations.py::TestTrashIsForLocalPathsOnly` 6 项（网络永不进回收站 /
  POSIX 上同样不进 / 目录递归 / 本地照旧进 / Shift+Del 不碰回收站 / `\\?\UNC\` 先还原）
- 变异验证 4 个全部被抓住，且各自只被预期的那条用例抓住（`build/mutate_trash_scope.py`）

#### 📐 真机数据（ln6 阶段 1：SMB 列目录到底多慢）
- 判据在真 `/proc/mounts`（64 项）上全部命中；NAS RTT 0.49–0.58 ms
- 热缓存：CIFS 每条目 scandir 0.047 / isdir 0.049 / stat 0.059 ms；`drop_caches=2` 后
  0.09–0.20 ms/条目，**冷/热比只有 1.3–1.6**；固定开销是每个目录 first-touch 的 2–4 ms
- 结论：**列一个 SMB 目录不是卡顿的来源**（19–69 条目的目录冷启也只要 3.7–6.3 ms），
  真正贵的是逐条目的属性拉取叠加在高 RTT 链路上 —— 下一步拿 NFS/gvfs 与真 wifi 链路复测
