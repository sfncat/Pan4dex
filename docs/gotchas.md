...
v1.9.021 修 L2「能取消 / 切走窗格不继续扫」时踩到两个坑，都记下来，因为两者都能单独复现。
...
### 55. 刷新一个空目录后看不到刚进去的文件：用 `if 集合:` 守卫「复位状态」时，边界空集会静默跳过复位

用户现场（v1.9.020）：新建目录→进去（空）→复制一个文件进去→状态栏「0 个目录, 1 个文件」
但列表恒空，F5 刷新也没用。状态栏走 `os.scandir` 直接数盘、与模型无关（所以计数永远对），
视图走 `DirStoreModel` 的行，两条路各走各的。

根因在 `dir_model._reload_top`：清行块写成 `if node.loaded and node.entries:`，空目录
`entries == []` 是假值→整块跳过→`node.loaded` 没被复位；新枚举回到 `_on_entries_loaded`，
却在幂等守卫 `if node.loaded: return`（本意防同一代次重复插行）处被整份丢弃。**一个「有数据
才处理」的真值判断，被复用去守一段「无论如何都要做的状态复位」，空集这个边界值就漏了。**

修法：把「发 Qt 行移除信号」（只在确有旧行时发）与「复位 `loaded`/`entries`」（无条件做）
拆开，给 `if` 补一个 `else` 分支。**教训**：`if collection:` 为假时，集合可能是「空」也可能
...
- 新 UI 模块**没有入口就别进版本库**，更别在功能清单里标 🟢；接线是它「存在」的一部分。已经只
  建了组件的，登记成待办（本仓现状见 `docs/todo.md` T2/T4/T5）。
